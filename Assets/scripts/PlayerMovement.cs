...
public class PlayerMovement : MonoBehaviour
{
    ...
    void playerInputs(int subSteps) 
    {
        ...
        if (((Input.GetKey(KeyCode.W) && !UsesArrowKeys) || (Input.GetKey(KeyCode.UpArrow) && UsesArrowKeys)) &&canJump&&jumps>0)
        {
            jumps--;
            canJump = false;
    ...
    void resolveTerrainCollisions()
    {
      ...
      //  grounded = false;
       ...
       // //Debug.Log("1 " + getTile(pos));
        if (getTile(new Vector2(pos.x, pos.y)) == 1)
        {
         //   //Debug.Log("1");
    ...
    {
        Vector3Int gridPosition = map.WorldToCell(new Vector3((int)inputPos.x, (int)inputPos.y,0));
